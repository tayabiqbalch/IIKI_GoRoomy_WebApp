...
namespace IIKI.GoRoomy.WebApp.Areas.BaseAppArea.Controllers
{
    ...
    public class RolePermissionsController : IIKI.GoRoomy.WebApp.Controllers.WebAppBaseController
    {
        ...
        public ActionResult AddEdit(string RoleName, string RoleId, int[] AreaRuleIds, int[] ModuleIds)
        {
            try
            {
                ...
                if (result1 == false)
                {
                    return RedirectToAction("Index", "RolePermissions", new { area = "BaseAppArea", response = "Rights Could not be Added" });
                }

                return RedirectToAction("Index", "RolePermissions", new { area = "BaseAppArea" });
