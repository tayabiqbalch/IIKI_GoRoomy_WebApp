...
{
    public class ProductDocumentsController : WebAppBaseController
    {
        // GET: ProductDocument
        public ActionResult Index(long Id)
        {
            try
            {
                var list = ProductDocumentBAL.GetList(Id);
                var Product = ProductBAL.Get(Id);
        ...
        public ActionResult _AddEditSubmitModal(int ProductId = 0)
        {
            try
            {
                var obj = ProductDocumentBAL.Get(0);
        ...
        [HttpPost]
        public ActionResult _AddEdit(ProductDocument oObject, string formaction = "save")
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    ////string FileBaseURL = IIKI.BaseApp.Common.CommonFunctions.GetConfigurationAppSettings("ProductDocumentsPath") + "/" + oObject.ProductId + "/";

                    string ActualURL = "/" + oObject.ProductId + "/";
                    string FileBaseURL = IIKI.BaseApp.Common.CommonFunctions.GetConfigurationAppSettings("ProductDocumentsPath") + ActualURL;
                    ...
                    if (Request.Files.Count > 0)
                    {
                        for (int i = 0; i < Request.Files.Count; i++)
                        {
                            ...
                            if (file != null && file.ContentLength > 0)
                            {
                                ...
                                oObject.DocumentId = DocumentId;
                                DocumentId = ProductDocumentBAL.Add(oObject);
                                ...
                                return RedirectToAction("Index", new { Id = oObject.ProductId });
        ...
        public ActionResult DeleteRecord(long DeleteRecordId)
        {
            try
            {
                var obj = ProductDocumentBAL.Get(DeleteRecordId);
